...
exports.totalcase = () => {
   var file = fs.readFileSync("../CynthiaAI.js").toString()
...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./AImedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./AImedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./AImedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./AImedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./AImedia/video/${filename}.mp4`), unlink(`./AImedia/gif/${filename}.gif`)])
	return buffer5
				   }
